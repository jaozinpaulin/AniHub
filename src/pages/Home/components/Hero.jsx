...
export default function Hero() {
    return (
        ...
        <section className="relative justify-center overflow-hidden bg-zinc-950">
            ...
            <div className="relative z-10 max-w-3xl pt-10 md:pt-6 text-center mx-auto md:text-left md:mx-0">

                <span className="inline-flex items-center gap-2 rounded-full border border-zinc-700 bg-zinc-900/70 px-3 py-1 md:py-2 text-xs sm:text-sm font-medium text-zinc-300 whitespace-nowrap">
                    <FaFire className="text-orange-500 text-sm" />
                    Plataforma em desenvolvimento
                </span>

                <h1 className="mt-4 sm:mt-2 text-3xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-5xl 2xl:text-6xl font-black text-white/90 leading-tight tracking-tight">
                ...
                <div className="mt-3 sm:mt-6 flex flex-row gap-3 sm:gap-4 w-full max-w-xs mx-auto justify-center md:max-w-min md:mx-0 md:justify-start">
                    ...
                    <Link to={'/explore'} className="flex-1">
                        <button className="w-full flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-2 sm:px-4 sm:py-2 lg:py-3 lg:px-5 text-xs sm:text-sm lg:text-base font-bold text-white hover:bg-blue-700 active:scale-98 transition-all cursor-pointer whitespace-nowrap shadow-lg shadow-blue-600/10">
                            Explorar Animes
                        </button>
                    </Link>

                    <Link to={'/categories'} className="flex-1">
                        <button className="w-full flex items-center justify-center rounded-xl border border-zinc-700 text-white px-4 py-2 sm:px-4 sm:py-2 lg:py-3 lg:px-5 text-xs sm:text-sm lg:text-base font-bold hover:bg-zinc-900 active:scale-98 transition-all cursor-pointer whitespace-nowrap">
                            Ver Categorias
                        </button>
                    </Link>
