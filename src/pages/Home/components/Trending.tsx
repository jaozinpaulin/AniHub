...
export default function Trending() {
    ...
    useEffect(() => {
        async function loadData() {
            try {
                ...
                console.error("Erro ao carregar animes da Home:", err);
            } finally {
    ...
    const topRatedAnimes = useMemo(() => {
        return [...animesHome].sort((a, b) => (Number(b.classificacao) || 0) - (Number(a.classificacao) || 0));
    }, [animesHome]);
    ...
    if (error) {
        return <ErrorMessage message={error} retry={loadAnimes} />;
    }

    if (animesHome.length === 0) {
        return (
            <EmptyState
                message="Nenhum anime encontrado no momento."
                retry={loadAnimes}
            />
        );
    }
                ...
                <div className="relative">
                    ...
                    <div
                        ...
                        {recentAnimes.map((a) => {
                            ...
                            return (
                                <Link key={idPlayer} to={`/Anime/${idPlayer}`} state={{ from: location.pathname }} className="shrink-0 group">
                                    ...
                                    <div className="w-36 sm:w-40 md:w-44 lg:w-48 overflow-hidden rounded-xl bg-zinc-800/70 transition-all duration-300 ease-in-out group-hover:bg-zinc-800 border border-transparent group-hover:border-zinc-600 hover:-translate-y-1">
                                        <div className="relative aspect-[3/4] overflow-hidden bg-zinc-900 flex items-center justify-center">
                                            ...
                                            <span
                                                className={`absolute top-2 left-2 px-2 py-1 rounded-md text-[10px] sm:text-xs font-medium text-white transition-colors duration-300 ${isDublado
